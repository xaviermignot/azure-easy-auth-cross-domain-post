resource "azuread_application" "easy_auth" {
  ...
  web {
    redirect_uris = [
      "https://web-${local.suffix}.azurewebsites.net/.auth/login/aad/callback",
      "https://${local.suffix}-api.${var.dns_zone_name}/.auth/login/aad/callback"]
